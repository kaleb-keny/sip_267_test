...
def test_10():
    ...
    #get some susd on di account and standard account
    self.eth_to_susd(bAccount[1],int(2e18))
    self.eth_to_susd(bAccount[2],int(2e18))
    ...
    try:
        ...
        history[-1].status==0, "tx did not revert, while it should have"
    
    self.swap_atomically(fromCurrencyKey='sUSD', toCurrencyKey='sETH', fromAmount=int(1e18), account=bAccount[2])    
    history[-1].status==1, "tx reverted, while it should not have reverted"
